...
(function () {
  ...
  function verifyRequest(method, url, header, data) {
    ...
    Object.keys(expectations.pairs).forEach(function (key) {
    ...
    notExpected.keys.forEach(function (key) {
      assert.ok(!pairs[key]);
    });
  ...
  function verifyHandlers(request) {
    expectations.handlers.forEach(function (event) {
  ...
  it('signature hashes all params alphabetically', function () {
    ...
    expectSignatureHashOf(
      'api_keykeybazbashflipflopfoobarmethodauth.getsessionsecret'
    );
  });
  ...
  it('signature hash treats undefined values as blank', function () {
    ...
    andParamsAre({
      signed: true,
      track: 'Replicating Networks',
      artist: 'Rabbit Milk',
      albumArtist: undefined,
    });
    expectSignatureHashOf(
      'albumArtistapi_keykeyartistRabbit Milkmethodany.methodtrackReplicating Networkssecret'
    );
  });
